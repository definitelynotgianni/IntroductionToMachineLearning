...
#Momentum
def new_update_net(Theta, nabla, eta, change):
    W, B = Theta
    Beta = 0.5
    for i in range(0, len(layers) - 1):
        W[i] = W[i] - (eta * nabla[0][i] + Beta * change)
        B[i] = B[i] - (eta * nabla[1][i] + Beta * change)
        c1 = (eta * nabla[0][i] + Beta * change)
        c2 = (eta * nabla[1][i] + Beta * change)
    new_Theta = (W, B)
    new_change = (c1, c2)
    return new_Theta, new_change
...
for i in range(0, epochs):
    ...
    for j in range(0, len(X_train), mini_batch_size):
        ...
        Theta, change = new_update_net(Theta, nabla, eta, change)
        ff_train = feedforward(X_train.T, Theta)
